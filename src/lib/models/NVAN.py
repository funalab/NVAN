...
class NVAN(nn.Module):
    ...
    def __init__(
        ...
        if self.multi_view_attention == self.context_based_attention:
            ...
            self.w_sc_i, self.w_sc_t = {}, {}
            for v in range(input_dim):
                self.w_sc_i[v] = nn.Conv2d(1, 1, (1, hidden_dim * 2 - self.k + 1), 1, 0)
                self.w_sc_t[v] = nn.Conv2d(1, 1, (1, hidden_dim * 2 - self.k + 1), 1, 0)
    ...
    def context_based_attention(self, hidden_matrix):
        ...
        h_t = []
        for v in range(self.input_dim):
            h_t.append(self.w_sc_t[v](hidden_matrix[:,v,-1].view(hidden_matrix.shape[0],1,1,-1)))
        # h_t: [batch, 1, 1, k]
        ...
        for t in range(time_point - 1):
            ...
            h_i = []
            for v in range(self.input_dim):
                h_i.append(self.w_sc_i[v](hidden_matrix[:,v,t].view(batch_size,1,1,-1)))
            h_i = torch.sum(torch.stack(h_i), dim=0)

            ''' cross-context '''
            h_ti = \
                self.w_cc_1(hidden_matrix[:,:,-1].view(batch_size,1,1,-1)) + \
                self.w_cc_2(hidden_matrix[:,:,t].view(batch_size,1,1,-1))
